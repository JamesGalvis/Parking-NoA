...
export async function getDailyChartData() {
  ...
  // Crear un arreglo con las ganancias por cada hora desde las 12:00am hasta las 11:00pm
  const earningsByHour = Array(24).fill(0); // 24 horas
  ...
  hourlyClients.forEach((client) => {
    if (client.exitDate) {
      const hour = DateTime.fromJSDate(client.exitDate).setZone(
        "America/Bogota"
      ).hour; // Obtener la hora en el rango 0-23
      if (hour >= 0 && hour <= 23) {
